...
// #define FLT float // FLT deve estar no intervalo [0..1]
// #define COR Vec3b
// #define CORF Vec3f
// #define GRY uchar
using namespace cv;
...
class SERVER {
  const std::string PORT = "3490"; // the port users will be connecting to
...
public:
  ...
  void sendVb(const std::vector<BYTE> &vb) {
    sendUint(vb.size());
    sendBytes(vb.size(), (BYTE *)vb.data());
  }

  void sendString(const std::string &st) {
    sendUint(st.length());
  ...
  void sendImg(const Mat_<COR> &img) {
    if (!img.isContinuous())
      erro("Erro sendImg: Imagem nao-continua");
    sendUint(img.rows);
    sendUint(img.cols);
  ...
  void sendImgComp(const Mat_<COR> &img) {
    if (!img.isContinuous())
      erro("Erro sendImgComp: Imagem nao-continua");
    std::vector<BYTE> vb;
  ...
  void receiveChar(char &ch) { receiveBytes(1, (BYTE *)&ch); }
  ...
  void receiveVb(std::vector<BYTE> &vb) {
    uint32_t t;
    receiveUint(t);
    vb.resize(t);
  ...
  void receiveString(std::string &st) {
    uint32_t t;
    receiveUint(t);
    st.resize(t);
...
class CLIENT {
  const std::string PORT = "3490"; // the port client will be connecting to
...
public:
  CLIENT(std::string endereco) {
    memset(&hints, 0, sizeof hints);
    hints.ai_family = AF_UNSPEC;
    hints.ai_socktype = SOCK_STREAM;
    if ((rv = getaddrinfo(endereco.c_str(), PORT.c_str(), &hints, &servinfo)) !=
        0) {
      fprintf(stderr, "getaddrinfo: %s\n", gai_strerror(rv));
      exit(1);
    }
...
bool testaVb(const std::vector<BYTE> vb, BYTE b) {
...
Mat_<GRY> grudaH(Mat_<GRY> a1, Mat_<GRY> a2, int nespaco, GRY backg) {
  int nc = a1.cols + nespaco + a2.cols;
  int nl = max(a1.rows, a2.rows);
...
Mat_<FLT> grudaH(Mat_<FLT> a1, Mat_<FLT> a2, int nespaco, FLT backg) {
  int nc = a1.cols + nespaco + a2.cols;
  int nl = max(a1.rows, a2.rows);
  Mat_<FLT> b(nl, nc, backg);
  for (int c = 0; c < a1.cols; c++)
    for (int l = 0; l < a1.rows; l++)
      b(l, c) = a1(l, c);
  for (int c = a1.cols + nespaco; c < nc; c++)
